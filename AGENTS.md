...
- [ ] **Broad differential parity, not a synthetic `.op` proxy:** the acceptance
      ...
      **Partial (2026-08-05):** `scripts/differential-parity.sh` +
      ...
      source-step OP / help ACstep) + Circuit_testing_v1/07_noise_rc_lowpass.asc
      authored `.noise` (R=10k C=10n; V(onoise) nRms=0; ≠ synthetic divider
      noise / BJT NoiseFigure / help NoiseStep) +
      Circuit_testing_v1/03_ac_rc_lowpass.asc authored `.ac` (R=1k C=100n;
      v(out) nRms=0 span≈0.998; ≠ synthetic RC_AC / ct noise RC) +
      ...
      8×C=10n; v(out) nRms=0 / nMax=0 span≈1.00; ≠ ct 03 single-pole /
      synthetic RC_AC) +
      ...
      (3φ SINE 170 V/60 Hz + RL line/load + 47u PF; v(a_load)/v(b_load)/
      ...
      adc/d_dff/dac; mid-clock strobes 01→11→10; continuous nRms≈0.01 /
      nMax≈1 edge-skew; ≠ ct 14 combinational / SampleAndHold) +
      Educational/MC1648.asc authored `.tran` (ECL VCO on-schematic NP/DD;
      v(out)/v(bias)/v(agc) nRms≈0.138/0.006/0.007 @ rmsTol=0.15/0.02;
      ...
      Educational/contrib/gr_del.asc authored `.ac` (all-pass lattice
      midnodes v(n005)/v(n006)/v(n008) nRms≈0; gd1/gd2 |V|≈1 hollow
      deferred).
      ...
      (domain-safe ±0.99 landed); divide2 / inverter deferred (`.machine` hollow). Resources mextram deferred (no authored analysis).
