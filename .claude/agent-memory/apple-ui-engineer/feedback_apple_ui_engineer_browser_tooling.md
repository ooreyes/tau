...
**When the browser MCPs are unavailable, use Playwright directly.** Seen 2026-08-17: `mcp__claude-in-chrome__*` returned "Claude in Chrome is not connected", and `preview_start` only serves the three Vite entries in `.claude/launch.json`, so neither could open a standalone HTML file. Working fallback, no repo files touched: serve the folder with `python3 -m http.server <port> --bind 127.0.0.1`, then drive `chromium` from a script in the scratchpad. Node will not resolve `playwright` by bare name from outside the repo, so import it by absolute path out of `node_modules/.pnpm/playwright@<ver>/node_modules/playwright/index.mjs`. `newContext({ colorScheme })` gets both themes in one run, and `locator(sel).screenshot()` per module plus a `page.evaluate` returning `getBoundingClientRect`/`getComputedStyle` gives measured evidence rather than eyeballed evidence, which is what [[tau-uiux-evidence-bar]] asks for.
