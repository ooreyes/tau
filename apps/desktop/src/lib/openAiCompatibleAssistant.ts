...
/**
 * Everything that differs between two OpenAI-chat-completions-shaped backends.
 * The transport, tool wiring, inspect round-trips, plan-repair loop, and strict
 * parsing below are provider-neutral and must stay that way: a second copy of
 * this machinery is how the Anthropic and local prompts drifted apart.
 */
export interface ChatProviderProfile {
  /** Stable provider id surfaced on AssistantProvider.id. */
  readonly id: string;
  readonly endpoint: string;
  /** Generation ceiling for this backend. */
  readonly maxTokens: number;
  readonly timeoutMs: number;
  /** Resolves a Tau preset id (or `custom:<name>`) to the backend's model name. */
  resolveModel(model: string): string;
  /** Per-request auth headers. Loopback backends contribute none. */
  authHeaders?(): Record<string, string>;
  /** Local chat templates accept MLX's thinking switch; hosted APIs reject it. */
  readonly sendChatTemplateKwargs: boolean;
  /** Hosted APIs reject unknown top-level fields such as parallel_tool_calls. */
  readonly sendParallelToolCallsFlag: boolean;
  /** Qwen-style suppression marker, or "" for models without one. */
  readonly thinkingMarker: string;
  /** Opening self-description in the system prompt. */
  readonly assistantLabel: string;
  /**
   * Whether to instruct the model to fall back to a bare JSON tool call. Only
   * backends observed to drop native tool_calls should enable this.
   */
  readonly allowTextToolFallback: boolean;
  /** Provider-specific user-facing error wording. */
  readonly wording: {
    /** e.g. "the local model" - substituted into invalid-response messages. */
...
export interface OpenAiCompatibleAssistantOptions {
  ...
  fetchImpl?: FetchLike;
  /** Test seam; production uses the profile's bounded deadline. */
  timeoutMs?: number;
}
...
${wrapAssistantContextForPrompt(contextText)}${profile.thinkingMarker ? `\n${profile.thinkingMarker.trim()}` : ""}`;
}
...
function visibleText(value: unknown, subject: string): string {
  if (value === null || value === undefined) return "";
  if (typeof value !== "string") {
    throw new AssistantProviderError("invalid_response", `The ${subject} returned an invalid message body.`);
  }
...
function parseToolCalls(value: unknown, allowCurrentApply: boolean, subject: string): {
  calls: ParsedPlanCall[];
  rejected: number;
} {
  if (value === undefined) return { calls: [], rejected: 0 };
  if (!Array.isArray(value)) {
    throw new AssistantProviderError("invalid_response", `The ${subject} returned malformed tool calls.`);
  }
...
function extractAssistantMessage(value: unknown, subject: string): ExtractedMessage {
  const response = record(value);
  const choices = response?.choices;
  if (!Array.isArray(choices) || choices.length === 0) {
    throw new AssistantProviderError("invalid_response", `The ${subject} returned no completion choice.`);
  }
  const choice = record(choices[0]);
  const message = record(choice?.message);
  if (!message || message.role !== "assistant") {
    throw new AssistantProviderError("invalid_response", `The ${subject} returned an invalid assistant message.`);
  }
...
function parseCompletion(value: unknown, allowCurrentApply: boolean, subject: string): ParsedCompletion {
  const { message, finishReason } = extractAssistantMessage(value, subject);
  const calls = parseToolCalls(message.tool_calls, allowCurrentApply, subject);
  ...
  return {
    reply: {
      text: fallback.consumed ? "" : visibleText(message.content, subject),
      actions,
...
function classifyFetchError(error: unknown, profile: ChatProviderProfile): AssistantProviderError {
  if (error instanceof AssistantProviderError) return error;
  if (error instanceof DOMException && error.name === "AbortError") {
    return new AssistantProviderError("aborted", profile.wording.aborted(), { cause: error });
  }
  if (error instanceof Error && error.name === "AbortError") {
    return new AssistantProviderError("aborted", profile.wording.aborted(), { cause: error });
  }
  if (error instanceof TypeError) {
    return new AssistantProviderError("offline", profile.wording.unreachable(), { cause: error });
  }
  return new AssistantProviderError("unknown", profile.wording.failed(), { cause: error });
}

/** OpenAI-chat-completions provider, non-streaming, driven entirely by a
 * ChatProviderProfile. It returns proposals only; file/canvas mutation remains
 * entirely outside this interface. */
export class OpenAiCompatibleAssistant implements AssistantProvider {
  readonly id: string;
  readonly model: string;
  readonly endpoint: string;
  protected readonly profile: ChatProviderProfile;
  private readonly fetchImpl: FetchLike;
  private readonly timeoutMs: number;

  constructor(profile: ChatProviderProfile, options: OpenAiCompatibleAssistantOptions = {}) {
    this.profile = profile;
    this.id = profile.id;
    this.model = options.model ?? "";
    this.endpoint = profile.endpoint;
    this.fetchImpl = options.fetchImpl ?? globalThis.fetch.bind(globalThis);
    this.timeoutMs = options.timeoutMs ?? profile.timeoutMs;
  }
  ...
  async complete(request: AssistantProviderRequest, signal?: AbortSignal): Promise<AssistantProviderReply> {
    ...
    const baseMessages: ChatPayloadMessage[] = [
      { role: "system", content: systemPrompt(request.contextText, allowCurrentApply, this.profile) },
      ...request.history.map(({ role, content }) => ({ role, content })),
    ];
    const body = {
      model: this.profile.resolveModel(this.model),
      messages: baseMessages,
      tools,
      tool_choice: "auto",
      ...(this.profile.sendParallelToolCallsFlag ? { parallel_tool_calls: false } : {}),
      stream: false,
      temperature: 0.2,
      max_tokens: this.profile.maxTokens,
      ...(this.profile.sendChatTemplateKwargs ? { chat_template_kwargs: { enable_thinking: false } } : {}),
    };
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        let messages: ChatPayloadMessage[] = repairHint
          ? [...baseMessages, {
              ...
              content: useTextToolFallback
                ? `The server dropped your native tool call. Emit only the plain JSON object {"name":"${TAU_CIRCUIT_PLAN_TOOL_NAME}","arguments":{...}} with one complete corrected plan. Do not use tool_call tags, prose, or markdown.${this.profile.thinkingMarker}`
                : `Tau rejected the prior logical plan: ${enrichRepairHint(repairHint)} Correct only that plan and return one complete ${TAU_CIRCUIT_PLAN_TOOL_NAME} call.${this.profile.thinkingMarker}`,
            }]
          : baseMessages;
        ...
        for (;;) {
          ...
          const response = await this.fetchImpl(this.endpoint, {
            method: "POST",
            headers: { "Content-Type": "application/json", ...(this.profile.authHeaders?.() ?? {}) },
            body: JSON.stringify(requestBody),
            signal: requestController.signal,
            credentials: "omit",
            // Neither a loopback server nor a pinned HTTPS host may redirect
            // circuit context to an origin the CSP never approved.
            redirect: "error",
          });
          if (!response.ok) {
            throw new AssistantProviderError("server", this.profile.wording.httpStatus(response.status));
          }
          try {
            json = await response.json() as unknown;
          } catch (error) {
            throw new AssistantProviderError("invalid_response", this.profile.wording.invalidJson(), { cause: error });
          }
          ...
          const { message } = extractAssistantMessage(json, this.profile.wording.subject);
        ...
        const parsed = parseCompletion(json, allowCurrentApply, this.profile.wording.subject);
      ...
      if (lastReply && repairHint && lastReply.actions.length === 0) {
        return {
          ...lastReply,
          text: lastReply.text || `Tau could not validate the ${this.profile.wording.subject}'s circuit proposal: ${repairHint}`,
        };
      }
      ...
      if (timedOut) {
        throw new AssistantProviderError(
          "server",
          this.profile.wording.timedOut(Math.max(1, Math.round(this.timeoutMs / 1_000))),
          { cause: error },
        );
      }
      throw classifyFetchError(error, this.profile);
    } finally {
