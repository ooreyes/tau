    ...
    for (const net of plan.nets) {
      ...
      for (let i = 0; i < endpoints.length; i += 1) {
        for (let j = i + 1; j < endpoints.length; j += 1) {
          ...
          // translates the component once per pass, which is how a flip-flop
          // with PRE, CLR and COM all on the ground net walked 960 units off
          // its column and shorted three signals together on the way.
