            ...
            <div className="settings-field-grid">
              <div className="settings-field" role="group" aria-label="Assistant path">
                ...
                <div className="settings-path-toggle" role="radiogroup" aria-label="Assistant path">
                  <button type="button" role="radio" aria-checked={pathChoice === "on-device"} className={`settings-path-btn${pathChoice === "on-device" ? " active" : ""}`} onClick={() => saveAssistantPreferences({ ...assistantPreferences, provider: "local-mlx" })}>On-device</button>
                  <button type="button" role="radio" aria-checked={pathChoice === "cloud"} className={`settings-path-btn${pathChoice === "cloud" ? " active" : ""}`} onClick={() => saveAssistantPreferences({ ...assistantPreferences, provider: assistantPreferences.provider === "anthropic" ? "anthropic" : "gemini" })}>Cloud</button>
                </div>
                <span className="settings-field-hint">{pathChoice === "on-device" ? "Stays on this Mac. Tau downloads and loads a small model for you — no account." : "Uses a free Gemini key or Anthropic key you provide. Circuit questions leave this Mac only after you consent."}</span>
              </div>
              {pathChoice === "on-device" ? (
                <>
                  <label className="settings-field" htmlFor="assistant-local-model">
                    <span>Model</span>
                    <select id="assistant-local-model" className="settings-select" aria-label="On-device model" value={assistantPreferences.localModel} onChange={(event) => saveAssistantPreferences({ ...assistantPreferences, localModel: event.currentTarget.value as LocalAiPresetInfo["id"] })}>
                      {localPresets.map((preset) => (<option key={preset.id} value={preset.id}>{preset.label}</option>))}
                    </select>
                    <span className="settings-field-hint">Start with 1.7B for a quick try (~900 MB). 4B is better for building circuits if you have 8 GB+ memory.</span>
                  </label>
                  <div className="settings-local-runtime" data-state={localAiStatus?.state ?? "checking"}>
                    ...
                    {localAiStatus && !selectedLocalPreset.downloaded && localAiStatus.state !== "ready" && (
                      <span className="settings-local-download">{selectedLocalPreset.downloadMb > 0 ? `Download: ${selectedLocalPreset.downloadMb.toLocaleString("en-US")} MB` : "Download size depends on the imported model."}</span>
                    )}
                    ...
                    {localAiStatus && (
                      <div className="settings-local-actions">
                        {localAiStatus.state === "ready" || localAiStatus.state === "starting" ? (
                          <Button size="sm" variant="outline" disabled={localAiBusy || !localAiStatus.managed} onClick={() => void runLocalAiAction(stopLocalAi)}>Turn off</Button>
                        ) : (
                          <Button size="sm" disabled={localAiBusy} onClick={() => void turnOnLocalAi()}>{localAiBusy ? "Working…" : !localAiStatus.installed ? "Turn on" : selectedLocalPreset.downloaded ? "Turn on" : "Download & turn on"}</Button>
                        )}
                      </div>
                    )}
                  </div>
                  <details className="settings-advanced" open={showAdvancedLocal || customLocalModels.length > 0} onToggle={(event) => setShowAdvancedLocal(event.currentTarget.open)}>
                    <summary>Advanced</summary>
                    <div className="settings-field" aria-label="Custom local models">
                      <span>Import another model</span>
                      <div className="settings-inline-actions">
                        <Input value={customModelRepository} aria-label="Hugging Face model repository" placeholder="owner/model-name" onChange={(event) => setCustomModelRepository(event.currentTarget.value)} />
                        <Button size="sm" variant="outline" disabled={!customModelRepository.trim()} onClick={() => { try { const models = importCustomLocalAiModel(customModelRepository); const imported = models.find((model) => model.repository === customModelRepository.trim()); setCustomLocalModels(models); setCustomModelRepository(""); if (imported) saveAssistantPreferences({ provider: "local-mlx", localModel: imported.id }); onNotice("Model imported. Choose Download & turn on to fetch it."); } catch (error) { setLocalAiError(userFacingErrorMessage(error, "Could not import that model.")); } }}>Import</Button>
                        {selectedLocalPreset && "custom" in selectedLocalPreset && (
                          <Button size="sm" variant="outline" onClick={() => { if (!window.confirm(`Remove ${selectedLocalPreset.label} from Tau? Downloaded files stay on disk.`)) return; setCustomLocalModels(removeCustomLocalAiModel(selectedLocalPreset.id)); saveAssistantPreferences({ provider: "local-mlx", localModel: "qwen3-4b-4bit" }); onNotice("Removed custom model from Tau."); }}>Remove</Button>
                        )}
                      </div>
                      <span className="settings-field-hint">Optional. Paste an MLX model name if you already know one — Tau handles the rest.</span>
                    </div>
                  </details>
                </>
              ) : (
                <>
                  <label className="settings-field" htmlFor="assistant-cloud-provider">
                    ...
                    <select id="assistant-cloud-provider" className="settings-select" aria-label="Cloud provider" value={assistantPreferences.provider} onChange={(event) => saveAssistantPreferences({ ...assistantPreferences, provider: event.currentTarget.value as AssistantProviderChoice })}>
                      <option value="gemini">Gemini · free tier</option>
                      <option value="anthropic">Anthropic · your API key</option>
                    </select>
                    <span className="settings-field-hint">Gemini Flash is free for students (no credit card). Anthropic bills separately from any ChatGPT plan.</span>
                  </label>
                  {assistantPreferences.provider === "gemini" && (
                    <label className="settings-field" htmlFor="assistant-gemini-model">
                      <span>Gemini model</span>
                      <select id="assistant-gemini-model" className="settings-select" aria-label="Gemini model" value={assistantPreferences.geminiModel} onChange={(event) => saveAssistantPreferences({ ...assistantPreferences, geminiModel: event.currentTarget.value })}>
                        {Object.entries(GEMINI_MODEL_PRESETS).map(([id, preset]) => (<option key={id} value={id}>{preset.freeTier ? `${preset.label} (free tier)` : preset.label}</option>))}
                      </select>
                    </label>
                  )}
                  <label className="settings-consent" htmlFor="assistant-cloud-consent">
                    <input id="assistant-cloud-consent" type="checkbox" checked={cloudConsent.consented} onChange={(event) => saveCloudAiConsent({ consented: event.currentTarget.checked })} />
                    <span>I understand circuit questions will be sent to {assistantPreferences.provider === "gemini" ? "Google" : "Anthropic"} for AI replies.</span>
                  </label>
                  {assistantPreferences.provider === "gemini" ? (
                    <label className="settings-field" htmlFor="assistant-gemini-key">
                      <span>Gemini API key</span>
                      <Input id="assistant-gemini-key" aria-label="Gemini API key" type="password" variant="mono" autoComplete="off" spellCheck={false} placeholder="Optional until you chat — AIza…" value={geminiKeyInput} onChange={(event) => { const next = event.currentTarget.value; setGeminiKeyInput(next); saveGeminiApiKey(next); }} />
                      <span className="settings-field-hint">Free key at aistudio.google.com/apikey. Stored in your Mac keychain — never in the schematic file.</span>
                    </label>
                  ) : (
                    <label className="settings-field" htmlFor="assistant-api-key">
                      <span>Anthropic API key</span>
                      <Input id="assistant-api-key" aria-label="Anthropic API key" type="password" variant="mono" autoComplete="off" spellCheck={false} placeholder="Optional until you chat — sk-ant-…" value={apiKeyInput} onChange={(event) => { const next = event.currentTarget.value; setApiKeyInput(next); saveAssistantApiKey(next); }} />
                      <span className="settings-field-hint">Stored in your Mac keychain. A ChatGPT subscription does not cover this key.</span>
                    </label>
                  )}
                  {!cloudConsent.consented && (<span className="settings-local-notice" role="status">Consent is required before cloud chat can send circuit context.</span>)}
                </>
              )}
            </div>
          </div>
