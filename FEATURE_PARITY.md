...
- 🟡 **EveryCircuit library gaps (honest, 2026-08-05)** — Tau has DC/AC V/I, pulse, ground, R/pot, C/**polarized C**/L/**bulb**, transformer/**CT transformer**, tline, switch/**push-button**/**SPDT**/**relay**, **motor**, opamp, comparator, E/F/G/H, B, diode/LED/zener/**photodiode**, BJT/MOS/JFET, digital gates, **logic constant**, dflop/**SR latch**/**T FF**/**JK FF**, sample-hold, modulator, subckt, testpoint. OP + post-`.tran` schematic current mode (labels + flow; **Live scrub through real `.tran` samples**; **scope cursor → `readoutTime`**; **editor + simulator** after a successful run — not simulator-only; Circuit header **Current mode** + **Live** badges). **Palette grouping polish:** explicit `PALETTE_SECTIONS` browse order Sources → Passives → Semiconductors → Analog → Digital → Electromechanical → Markers (no longer derived from `Set(CATALOG)` — `logicConstant` mid-Sources used to hoist Digital above Semiconductors); within-section order puts polarized C with caps, photodiode with diodes, latches/flops together, switches→relay→motor→CT xfmr). **Polarized cap** = honest SPICE `C` with electrolytic glyph (+ on pin a). **Logic constant** = honest DC `V` at 0/1 (or explicit volts). **Push-button** = static SPST contact (`R` 1m/1T, open/pressed). **SPDT** = COM↔NO / COM↔NC mutually exclusive contacts. **Photodiode** = silicon `D` + parallel photocurrent `Iph` (K→A; value = Iph). **Bulb** = cold filament `R` (I²R power via same path as any R). **Relay** = coil `R` + contact `S` (TAU_SW gated by coil V). **Motor** = armature series `R`+`L` only (no back-EMF). **SR latch** = LTspice `Digital\srflop` / XSPICE `d_dff` async set/reset (S→set, R→reset; no clock). **T / JK** = XSPICE `d_tff` / `d_jkff` (Tau-native; ASC lossy carrier — no LTspice `.asy`). **CT transformer** = primary `L` + two secondary half-windings (`L_full/4` each) + multi-winding `K` (same IdealTransformer-style coupling path; outer secondary dots for push-pull when CT grounded; ASC lossy carrier). **Hard-deferred (no fake IC):** 7-segment (needs UI-only display + honest drivers — not landed), 555 timer, ADC/DAC converters, counter/decoder ICs — refuse until a real Tau-owned subckt or explicitly limited behavioral model exists. Do not claim EveryCircuit library parity.
...
- 🟡 Comparators / logic gates / digital (LTspice `A` devices) —
  **Class-D's PWM stage does not need this path** (2026-08-04 re-measure):
  ...
  - **PHIDET landed (2026-08-02):** a two-DFF phase/frequency detector plus
    ...
    Counter and comparator-library symbols not present in the release corpus
    remain explicit unsupported cases (`Digital\srflop` now maps to `srflop`).
...
- 🟡 **Transient schematic current mode** (2026-08-05) — after a successful
  ...
  `tranComponentCurrents`). **Live continuous scrub (2026-08-05):** with **Live**
  on (default after `.tran`; Circuit header toggle), `readoutTime` loops through
...
- 🟡 Multiple plot panes and autorange — **landed for the
  ...
  **Bode phase manual Y limits landed** (2026-08-05): AcPlot lower-pane
...
- 🟡 Right-click trace → math/operations — **abs / negate / dB / uramp / sgn /
  ...
  **Noise legend right-click math landed** (2026-08-05):
...
- ✅ **Class-D fidelity — the flagship circuit now SIMULATES correctly
  ...
  **Differential matrix slice (2026-08-05):** `scripts/differential-parity.sh`
  ...
  (truth). Gap-closure → **pass=114 · sibling=5 · gap=0**: prior cells through
  ...
  nRms=0 / nMax=0 span≈1.04) plus Documents/LTspice/**Draft7.asc** authored
  `.ac` (series C + neg-R; v(vo) nRms=0 / nMax=0 span≈0.99) plus
  Documents/LTspice/**Draft2.asc** authored `.tran` (series C–R highpass;
  v(vout) nRms≈0.0062 nMax≈0.021 span≈0.20) plus Documents/LTspice/**Draft1.asc**
  authored `.tran` (series diode–L–R; v(n002)/v(n003) nRms≈0 nMax≈1e-4
  ...
  v(syn) nRms≈0.0078 nMax≈0.021 span≈1.57) plus Educational/**ISO16750-2_example.asc**
  ...
  ≠ Educational stepAC; v(z) nRms≈1e-9) plus LTspice.app help/**NoiseStep.asc**
  authored `.noise` (CE pair + 2N2222; list 10K→9.5–10.5k; .step R first=500;
  ≠ Educational stepnoise) plus LTspice.app Resources/**MicroCode.asc**
  authored `.tran` (BI Value+Value2 join; v(out)/v(out2) nRms≈6e-6) plus
  Circuit_testing_v1/**08_tran_rlc_ringing.asc** authored `.tran` (underdamped
  RLC; v(out)/v(in) nRms≈8e-4 / 0; ≠ synthetic RC_TRAN) plus
  Circuit_testing_v1/**04_dc_diode_curve.asc** authored `.dc` (1N4148 + 1k;
  v(anode)/i(v1) nRms≈1e-6; ≠ synthetic divider DC) plus
  ...
  source-step OP / help ACstep) plus
  Circuit_testing_v1/**07_noise_rc_lowpass.asc** authored `.noise` (R=10k
  C=10n; V(onoise) nRms=0; ≠ synthetic divider noise / BJT NoiseFigure / help
  NoiseStep) plus
  Circuit_testing_v1/**03_ac_rc_lowpass.asc** authored `.ac` (R=1k C=100n;
  v(out) nRms=0 span≈0.998; ≠ synthetic RC_AC C=1u/dec10/100k / ct noise RC) plus
  ...
  span≈1.00; ≠ Educational opamp.sub / ct 03 / ct 11) plus
  Circuit_testing_v1/**18_full_bridge_power_supply.asc** authored `.tran`
  (1N4007 bridge + 2200u Rser=80m + RLOAD=100; v(vdc)/v(ac1) nRms≈1e-4;
  ...
  .meas VOUT_AVG/VOUT_PP relErr≤2%; ≠ ct 18 bridge / edu 100W IRFP / ct 13 boost) plus
  Circuit_testing_v1/**13_boost_converter.asc** authored `.tran`
  (QS6K1 VDMOS + 1N5819 Schottky + L/C Rser; v(out) nRms=0 / nMax≈0.0015;
  .meas VOUT_AVG/VOUT_PP relErr≤2%; ≠ ct 12 buck / ct 18 bridge / edu 100W) plus
  ...
  edge-skew; ≠ ct 14 combinational / SampleAndHold) plus
  ...
  domain-safe ±0.99 stand-in; authored ±1.01 poles deferred; v(n001)/v(n002)
  nRms≈0 span≈4.94).
  ct 19 INA `.op` deferred (LTspice OP fails on same-deck tanh B_U*).
  gr_del gd outs still
  deferred (all-pass |V|≈1
  hollow). TwoTau non-rational Laplace deferred (Draft8 rational dual-deck landed). Draft6 AD823 /
...
- 🟡 Resolve a real device-model set — **common LTspice standard diodes/
  ...
  IRFP9240 bundled** (`engine/standardModels.ts`,
...
- ✅ **One circuit-aware assistant in both modes (2026-07-14):** a single
  ...
  **Packaged ngspice before AI apply (2026-08-05):** Create/Apply is fail-closed
...
- 🟡 **Panel migrations** (one commit each, screenshot-verified):
  ...
  **IndependentSourceEditor Waveform `ui/Select` (2026-08-05 follow-up):**
  ...
  (Settings untouched). **Simulation setup dialog `ui/Select` (2026-08-05):**
  all seven native `<select>`s in `SimulationSetupDialog` → dense
