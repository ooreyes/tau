...
- 🟡 **EveryCircuit library gaps (honest, 2026-08-05)** — Tau has DC/AC V/I, pulse, ground, R/pot, C/**polarized C**/L, transformer, tline, switch/**push-button**/**SPDT**, opamp, comparator, E/F/G/H, B, diode/LED/zener/**photodiode**, BJT/MOS/JFET, digital gates, **logic constant**, dflop, sample-hold, modulator, subckt, testpoint. OP + post-`.tran` schematic current mode (labels + flow; **scope cursor → `readoutTime`**). **Polarized cap** = honest SPICE `C` with electrolytic glyph (+ on pin a). **Logic constant** = honest DC `V` at 0/1 (or explicit volts). **Push-button** = static SPST contact (`R` 1m/1T, open/pressed). **SPDT** = COM↔NO / COM↔NC mutually exclusive contacts. **Photodiode** = silicon `D` + parallel photocurrent `Iph` (K→A; value = Iph). **Not in palette (need deep work or refuse):** light bulb, motor, center-tapped transformer, relay, 7-segment, SR/T/JK latches, 555, ADC/DAC, counter/decoder ICs. Do not claim EveryCircuit library parity.
