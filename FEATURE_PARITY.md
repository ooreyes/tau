...
- 🟡 **EveryCircuit library gaps (honest, 2026-08-05)** — Tau has DC/AC V/I, pulse, ground, R/pot, C/**polarized C**/L/**bulb**, transformer/**CT transformer**, tline, switch/**push-button**/**SPDT**/**relay**, **motor**, opamp, comparator, E/F/G/H, B, diode/LED/zener/**photodiode**, BJT/MOS/JFET, digital gates, **logic constant**, dflop/**SR latch**/**T FF**/**JK FF**, sample-hold, modulator, subckt, testpoint. OP + post-`.tran` schematic current mode (labels + flow; **scope cursor → `readoutTime`**; **editor + simulator** after a successful run — not simulator-only; Circuit header **Current mode** badge). **Polarized cap** = honest SPICE `C` with electrolytic glyph (+ on pin a). **Logic constant** = honest DC `V` at 0/1 (or explicit volts). **Push-button** = static SPST contact (`R` 1m/1T, open/pressed). **SPDT** = COM↔NO / COM↔NC mutually exclusive contacts. **Photodiode** = silicon `D` + parallel photocurrent `Iph` (K→A; value = Iph). **Bulb** = cold filament `R` (I²R power via same path as any R). **Relay** = coil `R` + contact `S` (TAU_SW gated by coil V). **Motor** = armature series `R`+`L` only (no back-EMF). **SR latch** = LTspice `Digital\srflop` / XSPICE `d_dff` async set/reset (S→set, R→reset; no clock). **T / JK** = XSPICE `d_tff` / `d_jkff` (Tau-native; ASC lossy carrier — no LTspice `.asy`). **CT transformer** = primary `L` + two secondary half-windings (`L_full/4` each) + multi-winding `K` (same IdealTransformer-style coupling path; outer secondary dots for push-pull when CT grounded; ASC lossy carrier). **Hard-deferred (no fake IC):** 7-segment (needs UI-only display + honest drivers — not landed), 555 timer, ADC/DAC converters, counter/decoder ICs — refuse until a real Tau-owned subckt or explicitly limited behavioral model exists. Do not claim EveryCircuit library parity.
...
- 🟡 **Transient schematic current mode** (2026-08-05) — after a successful
  ...
  — never invented. Falls back to OP when no ok transient. Not live continuous
  EveryCircuit sim; SHIPPABLE? **NO**.
