...
- 🟡 **EveryCircuit library gaps (honest, 2026-08-05)** — Tau has DC/AC V/I, pulse, ground, R/pot, C/**polarized C**/L/**bulb**, transformer, tline, switch/**push-button**/**SPDT**/**relay**, **motor**, opamp, comparator, E/F/G/H, B, diode/LED/zener/**photodiode**, BJT/MOS/JFET, digital gates, **logic constant**, dflop, sample-hold, modulator, subckt, testpoint. OP + post-`.tran` schematic current mode (labels + flow; **scope cursor → `readoutTime`**; **editor + simulator** after a successful run — not simulator-only; Circuit header **Current mode** badge). **Polarized cap** = honest SPICE `C` with electrolytic glyph (+ on pin a). **Logic constant** = honest DC `V` at 0/1 (or explicit volts). **Push-button** = static SPST contact (`R` 1m/1T, open/pressed). **SPDT** = COM↔NO / COM↔NC mutually exclusive contacts. **Photodiode** = silicon `D` + parallel photocurrent `Iph` (K→A; value = Iph). **Bulb** = cold filament `R` (I²R power via same path as any R). **Relay** = coil `R` + contact `S` (TAU_SW gated by coil V). **Motor** = armature series `R`+`L` only (no back-EMF). **Not in palette (need deep work or refuse):** center-tapped transformer, 7-segment, SR/T/JK latches, 555, ADC/DAC, counter/decoder ICs. Do not claim EveryCircuit library parity.
