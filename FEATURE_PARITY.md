...
- ✅ **Class-D fidelity — the flagship circuit now SIMULATES correctly
  ...
  **Differential matrix slice (2026-08-04):** `scripts/differential-parity.sh`
  ...
  (truth). Widened to **pass=10 · sibling=5 · gap=3**: prior RC/divider cells
  plus RC `.meas`, expanded `.step param` RC family, Educational curvetrace
  nested DC, Educational NoiseFigure (Vsource `Rser` → series R). Remaining
  gaps: nested/temp/source `.step`, Colpitts AC, Class-D non-tran. DoD
