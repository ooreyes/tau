...
- ✅ **Class-D fidelity — the flagship circuit now SIMULATES correctly
  ...
  **Differential matrix slice (2026-08-05):** `scripts/differential-parity.sh`
  ...
  (truth). Gap-closure → **pass=43 · sibling=5 · gap=0**: prior cells through
  DCopPnt OP plus Educational audioamp/UHFpreamp, 1563.asc authored `.ac`
  Tow-Thomas (TowTom2.sub; v(n003)/v(n002) nRms=0), and LM308.asc authored
  `.tran` discrete BJT+JFET op-amp (LPNP→PNP; pins 6/3/2 nRms≈0.0004).
  HalfSlope Laplace stripped (not landed); SoftDiodeRecovery deferred; BandGaps
  dc-temp miss; LoopGain/Electrometer LT1001 OTA wall; MC1648 deferred (probe
  stack). Harness-slice gaps closed; DoD broad-differential box remains open —
  see AGENTS.md.
