# PDF-6 verification — pdf6-quick-1

7/10 check runs passed (7/10 items green in every theme and viewport).
...
| P6-05 | dark 1280x800 | **FAIL** | clean: 1 tab(s), 0 colour chip(s), 0 dot(s); after an edit: 0 chip(s), 0 dot(s). Budget: chips 0 always (was 1 per tab), dots 0 clean / 1 unsaved |
| P6-09 | dark 1280x800 | **FAIL** | cluster spans x=12..521 (traffic-light inset floor 70); collides with the mode toggle at x=537: false; truncates long names: true; reads "τp6-09-dark-1280x800.asc" |
| P6-10 | dark 1280x800 | PASS | 51 hint(s) across 58 palette item(s); 1 distinct left offset(s) (1160), spread 0px, budget 1px |
| P6-06 | dark 1280x800 | **FAIL** | button present: true; clean sheet health=error badge="3" name="Diagnostics: 3 problems, this circuit will not run"; toggle opens+closes the window: false ({"before":false,"opened":false,"closed":false,"works":false}); no-ground run health=error (must be error - it will not run); policy persists: {"severityPolicy":"errors-only"} |
...
| P6-08 | dark 1280x800 | PASS | 40 moves in 430ms (10.8ms/move, budget 12); 40 distinct live widths (floor 20); settled at 380px vs last live 380px |
