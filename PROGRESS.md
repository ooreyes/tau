...
## 2026-08-24 - schematic workspace and multi-sheet project redesign

**What I did:** migrated the complete reviewed UI lineage onto
`auto/ltspice-parity`; integrated two isolated Luna High lanes for centered
schematic chrome and project-sheet contracts; ran three Sol closure cycles;
verified the final UI in Chrome and the packaged macOS app; sent Omar the
requested completion text; and began a separate deep simulation review.

**Files:** `apps/desktop/src/styles/schematicWorkspace20260824.css`,
`apps/desktop/src/styles/schematicWorkspaceGeometry.ts`,
`apps/desktop/src/styles/projectSheets20260824.css`, project hierarchy/interface
components and tests, `USB_PSU_WALKTHROUGH/`, and `BUCK_SUBCRCT_TEST/`.

**Tests:** desktop typecheck and full suite; web and packaged builds; design
drift; native fmt/clippy/tests and bundled-ngspice ignored tests; exact USB
deck; Chrome 900x600/desktop geometry; packaged sheet-contract and USB run
inspection.

**Parity items:** schematic mode control is centered; multi-sheet ports and
parent mappings are explicit and ordered; legacy geometry drift does not invent
contract drift; the recursive 120 VAC to 5 V project runs in the current
packaged app without flattening.

**Next step:** simulation review remediation, keeping the schematic design
language and explicit analysis provenance.
