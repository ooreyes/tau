...
- **Synced to origin:** auto/ltspice-parity @ fea580f (this unit's parent).
- **Claimed unit:** U3 - the attach-model-library UI plus preview-deck fidelity: `pickModelLibraryFile()` in fsBridge (Tauri dialog + fs with the readTextFile double byte-cap check, web `showOpenFilePicker` fallback), `ModelLibrariesDialog` (list with per-file size, remove, inline errors, pre-checks of the 64-file and aggregate caps with replace-in-place math), EditorToolbar `Library` button with count badge, CommandPalette "Model libraries..." entry, `unresolvedSubcktMessage` copy now pointing at the UI, and `userModelLibraries` threaded into SimulationPanel's export-netlist and assistantContext's netlist section.
- **Status:** DONE
- **Last completed sub-step:** Full gate matrix green on this exact tree: tsc clean; frontend 2036 passed / 135 files (10 new tests: dialog empty/attach/cancel/remove/at-cap/error, toolbar button + badge, export-netlist inlines an attached `.model`, assistant netlist section inlines an attached definition, updated unresolved-subckt copy); cargo test 28/0 + clippy clean; extended corpus 189/181/182 with both vendor-import proofs passing. No safety guard changed - the picker enforces the same double stat/read byte-cap as `readTextFile`, attachments still inline as literal text through the Rust `deck_lines` sanitizer, and the document validator caps are unchanged (the UI only pre-checks them for a cheaper inline error).
- **Demo examples (landed after U3):** `examples/class-d-amplifier/` (verbatim class-d_starter.asc + its deadtime.asy/.asc sub-block; imports through the project-aware pipeline, op-converges, and its own `.tran 0 3m` runs natively in ~0.8 s wall) and `examples/ad8541-buffer/` (Tau-native `.sim` with the real Analog Devices AD8541.lib pre-attached; proof asserts the vendor subckt inlines translated and the buffer tracks its input, plus the negative control without the library). Both proven by corpus-gated tests (`examplesClassD.corpus.ts`, `examplesAd8541Buffer.corpus.ts`) that ship-block if the examples break. `examples/README.md` walks the attach/remove/reattach Model libraries loop.
- **Hostile-review findings (fix unit next):** (1) `.asc` project import bypasses `validateSchematicDocument` - a crafted multi-MB `.asc` with tens of thousands of elements freezes the app on open and unbounded coordinates enter the store; route imported documents through the validator with a clean error. (2) Deck build resolves bundled models/subckts BEFORE user-attached libraries, so a same-named vendor model is silently discarded; flip precedence so an attached definition wins (LTspice semantics), with tests. Injection surface held: every native path goes through the Rust `deck_lines` allowlist; no route from attached-library text to execution or filesystem was found.
- **Hostile-review fixes (landed):** (1) `.asc` import now enforces document bounds - an early SYMBOL/WIRE/FLAG count gate inside the importer (before its quadratic pin-geometry phase, recursion included) plus full `validateSchematicDocument` at the App open path, with a corpus regression guard (new `schema-valid` column, floor 189/189) that immediately caught and drove out three validator rules real LTspice imports violate: the +-1e6 coordinate cap (hierarchical flattening parks sub-blocks at x >= 1e6 and nested parents compound it - measured 2.5e8 on a real 4-phase converter; cap now 1e12 with the reasoning documented), the orthogonal-wire requirement (LTspice permits diagonals; they flow through canvas and netlister), and the 160/1024-char value/directive caps (flattened battery models carry multi-KB PWL tables; both now 32 KiB single-field caps, aggregates still bounded by the 5 MB file and 512 KiB deck caps). (2) Deck resolution now prefers a user-attached vendor definition over a same-named Tau built-in (LTspice local-definition-wins; the old bundled-wins test was deliberately inverted), deterministic first-attached-wins across colliding attachments. Also de-NUL'd a literal control byte an old commit left in an ascImport.ts comment (the file diffed as binary). Gates re-verified after the fixes: tsc clean, frontend 2040/135 files, cargo 28/0 + clippy clean, corpus 189 imported / 181 op-converged / 182 deck-built / 189 schema-valid.
- **Next candidates:** commit the docs honesty pass (README vendor-model section refreshed, LICENSE dropped, KNOWN_ISSUES updated - staged alongside this unit), then the fresh release DMG with the Examples folder staged in.
